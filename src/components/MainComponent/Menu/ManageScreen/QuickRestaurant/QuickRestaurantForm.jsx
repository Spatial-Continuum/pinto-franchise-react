...
import {
  fetchQuickRestaurantApi,
  selectQuickRestaurant,
  selectApiLoading,
  selectApiError,
...
const QuickRestaurantForm = () => {
  ...
  console.log("ashdioiwlksldkfsd", singleRestaurant);
  const edit = location?.state.edit || false;
  const [formState, setFormState] = useState({
    title: singleRestaurant?.filter_name || "",
  ...
  const [cuisines, setCuisines] = useState(
    singleRestaurant?.cuisines?.map((sub) => ({
      id: sub.cuisine_id,
  ...
  const handleSubmit = (e) => {
    console.log("sadfsdsds", singleRestaurant);
    e.preventDefault();
    const formData = {};
    const restaurant_ids = selectedRestaurant.map(
      (items) => items.restaurant_id
    );

    formData["filter_name"] = formState.title;

    formData["restaurant_ids"] = restaurant_ids;

    let type = "post";
    let api = `${API_URL}/restaurant/menu/quickfilterrestaurant`;
    if (singleRestaurant && edit) {
      type = "put";
      api = `${API_URL}/restaurant/menu/quickfilterrestaurant/${singleRestaurant?.quickfilter_id}`;
    }
    axios[type](api, formData, {
      ...
      .then(function (response) {
        ...
        getInfo();
        navigate("/menu/manage-screen/show-quick-restaurant", {
          state: { restaurants: qrestaurants },
        });
      })
      .catch(function (error) {
        console.log(error);
  ...
  console.log("klafjasowjpojsdklflkjs", formState);
    ...
    >
      <div className="m-6">
        <h2 className="text-lg font-semibold">
          {" "}
          {edit ? "Update " : "Add "} Quick filter-Restaurant
        </h2>
            ...
            <div className="flex ">
              ...
              <div className="mb-6 ml-8 relative ">
                ...
                {restaurants.length > 0 && (
                  <ul className="absolute z-50 w-56 top-full border rounded bg-white shadow-md max-h-48 overflow-auto ">
