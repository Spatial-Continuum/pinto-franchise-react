...
{   
  ...
  return( 
    ...
    <div className="flex justify-between items-center mb-4">
              ...
              <button className="text-orange-500 text-sm" onClick={()=>{;navigate("/menu/manage-screen/show-authentic" , { state: { Authentic } });}} >View all</button>
            </div>
            <div className="flex flex-row gap-5">  
            
              {Authentic?.slice(0, Authentic.length > 4 ? 4 : Authentic.length).map((authentic) => ( 
              ...
              <CategoryCard add={true} isAdd={true} categories={Authentic} style={"w-56 h-42"}
              onEdit={()=>{navigate("/menu/manage-screen/categoty-form" , { state: { Authentic } })}} />
            </div> 
            </div>
  )
}
