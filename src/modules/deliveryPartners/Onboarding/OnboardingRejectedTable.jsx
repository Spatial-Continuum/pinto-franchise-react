...
import {
  fetchMerchantListOnboarding,
  selectMerchantListOnboarding,
  selectMerchantError,
...
const OnboardingRejectedTable = ({ searchTerm }) => {
  ...
  const handleLaunch = async () => {
    try {
      const response = await RestaurantService.updateRestaurantSuccess(
        currentRestaurantId
      );
  ...
  const sumbitRejection = async () => {
    try {
      const response = await RestaurantService.updateRestaurantRejected(
        currentRestaurantId,
        rejectReason
      );
  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          />
          <button onClick={() => handleShopNameClick(row.restaurant_id)}>
            {row.name}
          </button>
        </div>
      ),
      ...
      cell: (row) => (
        <div>
          <button
            style={{
              marginRight: "8px",
              backgroundColor: "rgba(247, 30, 129, 0.08)",
              color: "#FF00C7",
              borderColor: "#FF00C7",
              border: "1px solid",
              padding: "5px",
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-3/6 ">
            <div className="flex items-center">
              ...
              <div className="ml-4 flex-1">
                ...
                <div className="flex items-center text-sm text-gray-600">
                  <span>
                    {addressPopupData.primary_phone}&nbsp;-&nbsp;
                    {addressPopupData.secondary_phone}
                  </span>
                  <span className="ml-4">
                    ⏰Timings :&nbsp;
                    {getCurrentDayHours(addressPopupData.opening_hours)}
                  </span>
                </div>
              </div>
            ...
            <div className="border-[1px] border-gray-300 rounded-lg mt-4">
              <div className="  p-4">
                <h6 className="text-xl font-medium text-black mb-2">
                  Shop Address
                </h6>
                <h4 className="text-sm  text-gray-700">
                  {addressPopupData.street_address_1},
                  {addressPopupData.street_address_2}
                </h4>
                <h4 className="text-sm  text-gray-700">
                  {addressPopupData.landmark},
                </h4>
                <h4 className="text-sm text-gray-700">
                  {addressPopupData.city} - {addressPopupData.pincode}
                </h4>
              </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <div className="flex justify-between">
              <div className="mt-4">
                <button
                  className="px-4 py-2 text-gray-700 border border-gray-400 rounded-md hover:bg-gray-100"
