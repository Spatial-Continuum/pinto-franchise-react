...
export const fetchMerchantSearchApi = createAsyncThunk('api/fetchMerchantsearch', async ({ name }, { rejectWithValue }) => {
    console.log("andc", `${API_URL}/restaurant/merchant/search`);
    try{
        const response = await axios.get(`${API_URL}/restaurant/merchant/search`, {
            params: {name}
        });
...
export const getAllRestaurantPending = createAsyncThunk('restaurant/getAllRestaurantPendeing', async(_, { rejectWithValue }) =>{
    try{
      const response = await axios.get(`${API_URL}/restaurant/merchant/list`,{params:{onboarding}});
      return response.data
    }catch(error){
      return rejectWithValue(error.response?.data || error.message);
    }
  })
export const fetchMerchantListOnboarding = createAsyncThunk('api/fetchMerchnatListOnboarding', async ({onboarding }, {rejectWithValue }) => {
try{
    const response = await axios.get(`${API_URL}/restaurant/merchant/list`,{params:{onboarding},})
    return response.data;
}catch(error){
...
const merchantSlice = createSlice({
    ...
    extraReducers:(builder) => {
       ...
       .addCase(fetchMerchantSearchApi.fulfilled, (state, action) => {
        console.log('Fetched merchant data:', action.payload);
        state.loading = false;
       ...
       .addCase(fetchMerchantListOnboarding.fulfilled, (state, action) => {
        console.log('Fetched merchant onboarding data:', action.payload);
        state.loading = false;
        state.sucessMerchantData = action.payload;
       })
...
export const selectMerchantListOnboarding = (state) => state.merchant.sucessMerchantData
export const selectAllRestaurantsPending = (state) => state.merchant.allRestaurantPending;
