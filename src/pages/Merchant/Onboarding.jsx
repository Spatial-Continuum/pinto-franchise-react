...
import { getAllRestaurantPending, getAllRestaurantRejected, selectRejectedRestaurants, selectPendingRestaurants, getAllRestaurantSuccess, selectSuccessRestaurants } from "../../redux/slices/restaurant";
...
const Onboarding = () => {
  ...
  useEffect(() => {
    dispatch(getAllRestaurantPending());
    dispatch(getAllRestaurantRejected());
    dispatch(getAllRestaurantSuccess());
  }, [dispatch]);

  const cardsData = [
    { value: pendingRestaurants.length || 0, label: "Pending Approval", textColor: "text-[#1E99FF]", borderColor: "border-[#1E99FF]" },
    { value: rejectedRestaurants.length || 0, label: "Rejected", textColor: "text-[#FF6B00]", borderColor: "border-[#FF6B00]" },
    {value: successRestaurants.length || 0, label: "On boarded", textColor: "text-[#720F3D]", borderColor: "border-[#720F3D]" },
      ...
      <div>
        ...
        <div className="flex justify-between">
          ...
          <div className="flex gap-6 border-b pb-2 mb-5 mt-5 ">
            <div
              className={`cursor-pointer text-lg font-semibold rounded-sm ${selectedTab === "Pending" ? "text-black border-b-4 border-orange-600"  : "text-gray-500"}`}
              onClick={() => setSelectedTab("Pending")}
            >
              Pending
            </div>
            <div
              className={`cursor-pointer text-lg font-semibold rounded-sm ${selectedTab === "Rejected" ? "text-black border-b-4 border-orange-600" : "text-gray-500"}`}
          ...
          <div>
                {/* Search Box */}
          <SearchBox onSearch={handleSearch} placeholder="Search here" img={search} />
          </div>
        </div>
        {/* 
        {/* Conditional Rendering of Tables */}
        <div className="mb-44 ">
          {selectedTab === "Pending" && <OnboardingPendingTable searchTerm={searchTerm} />}
          {selectedTab === "Rejected" && <OnboardingRejectedTable searchTerm={searchTerm} />}
        </div>
      </div>
    </MainLayout>
  );
