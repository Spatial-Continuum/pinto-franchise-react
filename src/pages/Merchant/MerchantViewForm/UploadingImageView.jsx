...
const UploadingImageView = ({ restaurantInfo, onDataChange }) => {
  ...
  const [previews, setPreviews] = useState({
    fssaiPreview: restaurantInfo?.fssaiCertificate ? 'PDF file uploaded' : null,
    imagePreview: typeof restaurantInfo?.image === 'string' 
      ? restaurantInfo.image 
      : restaurantInfo?.image 
        ? URL.createObjectURL(restaurantInfo.image) 
        : null,
    logoPreview: typeof restaurantInfo?.logo === 'string' 
      ? restaurantInfo.logo 
      : restaurantInfo?.logo 
        ? URL.createObjectURL(restaurantInfo.logo) 
        : null,
  });
  ...
  useEffect(() => {
    return () => {
      if (previews.imagePreview && typeof restaurantInfo?.image !== 'string') {
        URL.revokeObjectURL(previews.imagePreview);
      }
      if (previews.logoPreview && typeof restaurantInfo?.logo !== 'string') {
